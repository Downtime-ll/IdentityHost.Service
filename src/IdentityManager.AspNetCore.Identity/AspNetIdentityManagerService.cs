...
namespace IdentityManager.AspNetCore.Identity
{
        ...
        where TRoleKey : IEquatable<TRoleKey>
    {
        ...
        AspNetIdentityManagerService(UserManager<TUser> userManager, RoleManager<TRole> roleManager, Func<string, TUserKey> parseUserSubject = null, Func<string, TRoleKey> parseRoleSubject = null)
        {
        ...
        {
            return user => userManager.GetClaimsAsync(user).GetAwaiter().GetResult().Where(x => x.Type == type).Select(x => x.Value).FirstOrDefault();
        }
        public virtual Func<TUser, string, IdentityManagerResult> SetForClaim(string type)
        {
            return (user, value) =>
            {
                var claims = this.userManager.GetClaimsAsync(user).GetAwaiter().GetResult().Where(x => x.Type == type).ToArray();
                foreach (var claim in claims)
                {
                    var result = this.userManager.RemoveClaimAsync(user, claim).GetAwaiter().GetResult();
                    if (!result.Succeeded)
                    {
                        return new IdentityManagerResult(result.Errors.First().IdentityErrorToString());
                    }
                }
                if (!String.IsNullOrWhiteSpace(value))
                {
                    var result = this.userManager.AddClaimAsync(user, new Claim(type, value)).GetAwaiter().GetResult();
                    if (!result.Succeeded)
                    {
                        return new IdentityManagerResult(result.Errors.First().IdentityErrorToString());
                    }
        ...
        {
            var token =  this.userManager.GeneratePasswordResetTokenAsync(user).GetAwaiter().GetResult();
            var result = this.userManager.ResetPasswordAsync(user, token, password).GetAwaiter().GetResult();
            if (!result.Succeeded)
            {
                return new IdentityManagerResult(result.Errors.First().IdentityErrorToString());
            }
        ...
        {
            return userManager.GetEmailAsync(user).GetAwaiter().GetResult();
        }
        public virtual IdentityManagerResult SetEmail(TUser user, string email)
        {
            var result = userManager.SetEmailAsync(user, email).GetAwaiter().GetResult();
            if (!result.Succeeded)
            {
                return new IdentityManagerResult(result.Errors.First().IdentityErrorToString());
            }
            ...
            {
                var token = this.userManager.GenerateEmailConfirmationTokenAsync(user).GetAwaiter().GetResult();
                result = this.userManager.ConfirmEmailAsync(user, token).GetAwaiter().GetResult();
                if (!result.Succeeded)
                {
                    return new IdentityManagerResult(result.Errors.First().IdentityErrorToString());
                }
        ...
        {
            return userManager.GetPhoneNumberAsync(user).GetAwaiter().GetResult();
        }
        public virtual IdentityManagerResult SetPhone(TUser user, string phone)
        {
            var result = this.userManager.SetPhoneNumberAsync(user, phone).GetAwaiter().GetResult();
            if (!result.Succeeded)
            {
                return new IdentityManagerResult(result.Errors.First().IdentityErrorToString());
            }
            ...
            {
                var token = this.userManager.GenerateChangePhoneNumberTokenAsync(user, phone).GetAwaiter().GetResult();
                result = this.userManager.ChangePhoneNumberAsync(user, phone, token).GetAwaiter().GetResult();
                if (!result.Succeeded)
                {
                    return new IdentityManagerResult(result.Errors.First().IdentityErrorToString());
                }
        ...
        {
            return userManager.GetTwoFactorEnabledAsync(user).GetAwaiter().GetResult();
        }
        public virtual IdentityManagerResult SetTwoFactorEnabled(TUser user, bool enabled)
        {
            var result = userManager.SetTwoFactorEnabledAsync(user, enabled).GetAwaiter().GetResult();
            if (!result.Succeeded)
            {
                return new IdentityManagerResult(result.Errors.First().IdentityErrorToString());
            }
        ...
        {
            return userManager.GetLockoutEnabledAsync(user).GetAwaiter().GetResult();
        }
        public virtual IdentityManagerResult SetLockoutEnabled(TUser user, bool enabled)
        {
            var result = userManager.SetLockoutEnabledAsync(user, enabled).GetAwaiter().GetResult();
            if (!result.Succeeded)
            {
                return new IdentityManagerResult(result.Errors.First().IdentityErrorToString());
            }
        ...
        {
            return userManager.GetLockoutEndDateAsync(user).GetAwaiter().GetResult() > DateTimeOffset.UtcNow;
        }
        public virtual IdentityManagerResult SetLockedOut(TUser user, bool locked)
        {
            if (locked)
            {
                var result = userManager.SetLockoutEndDateAsync(user, DateTimeOffset.MaxValue).GetAwaiter().GetResult();
                if (!result.Succeeded)
                {
                    return new IdentityManagerResult(result.Errors.First().IdentityErrorToString());
                }
            ...
            {
                var result = userManager.SetLockoutEndDateAsync(user, DateTimeOffset.MinValue).GetAwaiter().GetResult();
                if (!result.Succeeded)
                {
                    return new IdentityManagerResult(result.Errors.First().IdentityErrorToString());
                }
        ...
        public virtual async Task<IdentityManagerResult<CreateResult>> CreateUserAsync(IEnumerable<IdentityManager.PropertyValue> properties)
        {
            ...
            if (!result.Succeeded)
            {
                return new IdentityManagerResult<CreateResult>(result.Errors.Select(x=>x.IdentityErrorToString()).ToArray());
            }
        ...
        public virtual async Task<IdentityManagerResult> DeleteUserAsync(string subject)
        {
            ...
            var user = await this.userManager.FindByIdAsync(key.ToString());
            if (user == null)
            {
        ...
        public virtual async Task<IdentityManagerResult> SetUserPropertyAsync(string subject, string type, string value)
        {
            ...
            if (!result.Succeeded)
            {
                return new IdentityManagerResult(result.Errors.Select(x=>x.IdentityErrorToString()).ToArray());
            }
        ...
        public virtual async Task<IdentityManagerResult> AddUserClaimAsync(string subject, string type, string value)
        {
            ...
            {
                var result = await this.userManager.AddClaimAsync(user, new System.Security.Claims.Claim(type, value));
                if (!result.Succeeded)
                {
                    return new IdentityManagerResult<CreateResult>(result.Errors.Select(x=>x.IdentityErrorToString()).ToArray());
                }
        ...
        public virtual async Task<IdentityManagerResult> RemoveUserClaimAsync(string subject, string type, string value)
        {
            ...
            var result = await this.userManager.RemoveClaimAsync(user, new System.Security.Claims.Claim(type, value));
            if (!result.Succeeded)
            {
                return new IdentityManagerResult<CreateResult>(result.Errors.Select(x=>x.IdentityErrorToString()).ToArray());
            }
        ...
        public virtual async Task<IdentityManagerResult> DeleteRoleAsync(string subject)
        {
            ...
            var role = await this.roleManager.FindByIdAsync(key.ToString());
            if (role == null)
            {
                return new IdentityManagerResult("Invalid subject");
            }
